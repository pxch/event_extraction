...
parser.add_argument('--sample_type', default='one',
                    help='how to select samples, options: '
                         'one (one left event for every negative sample), or '
                         'all (every left event for every negative sample)')
parser.add_argument(
    '--pair_type', default='wo_arg',
    help='types of pair input, options: '
         'wo_arg: event w/ arg_i vs event w/o arg_i (imp_arg_i exists) and '
         '    event w/o arg_j vs event w/ arg_j (imp_arg_j does not exist), '
         'two_args: event w/ cand_a as arg_i vs event w/ cand_a as arg_j '
         '    (which cand_a is the true arg for imp_arg_i, and both imp_arg_i '
         '    and imp_arg_j exist)')
...
assert args.pair_type in ['wo_arg', 'two_args'], \
    'pair_type can only be wo_arg, two_args'
...
for input_f in input_files:
    with BZ2File(input_f, 'r') as fin:
        ...
        for script in script_corpus.scripts:
            ...
            if args.pair_type == 'wo_arg':
                pair_tuning_inputs = \
                    rich_script.get_pair_tuning_input_list_wo_arg(
                ...
                pair_tuning_inputs = \
                    rich_script.get_pair_tuning_input_list_two_args(
                        sample_type=args.sample_type)
